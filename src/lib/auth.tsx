...
const DEFAULT_PREFS: UserPreferences = {
  units: "kg", dateFormat: "MM/DD/YYYY",
  notifVet: true, notifWeight: true, notifFeeding: false,
...
interface AuthContextValue {
  ...
  logout: () => void;
  updateUser:      (data: Partial<Pick<User, "name" | "preferences" | "avatar" | "phone" | "country">>) => void;
  ...
  addSleepLog:     (catId: string, hours: number, activity: SleepLog["activity"], bcs: number, date: string) => void;
  ...
  updateVetRecord: (recordId: string, data: Partial<Omit<VetRecord, "id" | "catId" | "createdAt">>) => void;
...
function migrateUser(u: User): User {
  return {
    cats: [], weightLogs: [], sleepLogs: [],
    meals: [], vetRecords: [], hydrationLogs: [], chatMessages: [],
...
function hashPassword(p: string) { return btoa(encodeURIComponent(p)); }
function checkPassword(p: string, h: string) { return hashPassword(p) === h; }
...
const AuthContext = createContext<AuthContextValue>({
  ...
  addCat: () => {}, removeCat: () => {}, updateCat: () => {},
  addWeightLog: () => {}, removeWeightLog: () => {},
  addSleepLog: () => {},
  addMeal: () => {}, removeMeal: () => {}, updateMeal: () => {},
  addVetRecord: () => {}, removeVetRecord: () => {}, updateVetRecord: () => {},
  addHydrationLog: () => {},
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function login(email: string, password: string) {
    ...
    if (!checkPassword(password, found.passwordHash)) return { error: "Incorrect password. Please try again." };
  ...
  async function signup(name: string, email: string, password: string) {
    ...
    const newUser: User = migrateUser({
      ...
      cats: [], weightLogs: [], sleepLogs: [], meals: [], vetRecords: [], hydrationLogs: [], chatMessages: [],
      dismissedNotifications: [],
      preferences: DEFAULT_PREFS,
      createdAt: now(),
    });
  ...
  function logout() { saveSession(null); setUser(null); }

  function updateUser(data: Partial<Pick<User, "name" | "preferences" | "avatar" | "phone" | "country">>) {
    if (!user) return;
  ...
  async function changePassword(currentPassword: string, newPassword: string) {
    ...
    if (!checkPassword(currentPassword, user.passwordHash)) return { error: "Current password is incorrect." };
  ...
  function addSleepLog(catId: string, hours: number, activity: SleepLog["activity"], bcs: number, date: string) {
    if (!user) return;
  ...
  function addVetRecord(catId: string, record: Omit<VetRecord, "id" | "catId" | "createdAt">) {
    if (!user) return;
    commit({ ...user, vetRecords: [...user.vetRecords, { ...record, id: uid(), catId, createdAt: now() }] });
  }
  ...
  function updateVetRecord(recordId: string, data: Partial<Omit<VetRecord, "id" | "catId" | "createdAt">>) {
    if (!user) return;
    commit({ ...user, vetRecords: user.vetRecords.map((r) => r.id === recordId ? { ...r, ...data } : r) });
  }
