    ...
    >
      <div className="flex gap-1">
        {[0, 1, 2].map((i) => (
          <motion.div
            key={i}
            className={`flex-1 h-1 rounded-full ${i < score ? colors[score] : "bg-border"}`}
            animate={{ scaleX: i < score ? 1 : 0.95 }}
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap gap-x-3 gap-y-1">
          {checks.map((c) => (
            ...
            >
              <motion.div
                animate={c.pass ? { scale: [1, 1.4, 1], rotate: [0, 20, 0] } : {}}
        ...
        <span
          className={`text-[10px] font-medium shrink-0 ${colors[score].replace("bg-", "text-").replace("var(--warm-yellow)", "[var(--warm-yellow)]")}`}
        >
          {labels[score]}
        </span>
      </div>
...
function SignUp() {
  ...
  const passwordScore = [password.length >= 8, /[A-Z]/.test(password), /\d/.test(password)].filter(
    Boolean,
  ).length;
    ...
    <AuthShell>
      ...
      >
        <motion.div
          ...
          className="relative px-6 pt-14 pb-0 z-10 flex items-center gap-3"
        >
        ...
        <motion.div variants={childVariants} className="relative px-6 pt-8 pb-0 z-10">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Join thousands of cat parents tracking their pet's health.
          </p>
        </motion.div>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
                ...
                <div
                  className={inputRow(
                    showNameError
                      ...
                      : "border-transparent focus-within:border-[var(--coral)]",
                  )}
                >
              ...
              >
                <motion.button
                  ...
                  whileTap={{ scale: 0.88 }}
                  animate={
                    agreed ? { backgroundColor: "var(--coral)", borderColor: "var(--coral)" } : {}
                  }
              ...
              <motion.button
                type="submit"
                disabled={loading || passwordScore < 1}
        ...
        <motion.div variants={childVariants} className="px-6 py-8 text-center">
          <p className="text-sm text-muted-foreground">
            ...
            <Link
              to="/login"
              className="font-medium text-[var(--coral)] hover:underline underline-offset-2"
            >
              Sign in
            </Link>
          </p>
...
function SocialButton({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      whileHover={!disabled ? { y: -2, boxShadow: "0 4px 16px -6px rgba(0,0,0,0.15)" } : {}}
      whileTap={!disabled ? tapScale : {}}
      className="flex-1 flex items-center justify-center gap-2 bg-secondary hover:bg-[var(--coral-soft)] border border-border rounded-xl py-2.5 text-sm font-medium text-foreground transition-colors disabled:opacity-60"
    >
      {icon}
      {label}
    </motion.button>
