...
import { ArrowLeft, Bell, UtensilsCrossed, PawPrint, Plus, Trash2, Clock, ChevronUp, ChevronDown } from "lucide-react";
...
function TimePicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  function adj(type: "hour" | "min", dir: 1 | -1) {
    if (type === "hour") {
      ...
      setHour(h); commit(h, minute, period);
    } else {
      const m = dir === 1 ? (minute >= 55 ? 0 : minute + 5) : (minute <= 0 ? 55 : minute - 5);
      setMinute(m); commit(hour, m, period);
    }
  }

  function toggleP() {
    const p = period === "AM" ? "PM" : "AM";
    setPeriod(p); commit(hour, minute, p);
  }
        ...
        {open && (
          <>
            ...
            >
              ...
              >
                <p className="text-center text-xs text-muted-foreground uppercase tracking-wide mb-4">Select Time</p>
                <div className="flex items-center justify-center gap-3">
                  {/* Hour column */}
                  <div className="flex flex-col items-center">
                    <button type="button" onClick={() => adj("hour", 1)}
                  ...
                  {/* Minute column */}
                  <div className="flex flex-col items-center">
                    <button type="button" onClick={() => adj("min", 1)}
                      className="w-14 h-10 rounded-xl bg-secondary flex items-center justify-center active:scale-90 transition-transform">
                      <ChevronUp size={18} className="text-foreground" />
                    </button>
                    <span className="font-serif text-4xl font-semibold my-3 w-14 text-center leading-none">
                  ...
                  <div className="flex flex-col gap-2 ml-2">
                    <button type="button" onClick={() => { setPeriod("AM"); commit(hour, minute, "AM"); }}
                    ...
                    <button type="button" onClick={() => { setPeriod("PM"); commit(hour, minute, "PM"); }}
...
function MealCard({ meal, onDelete, onToggleReminder }: {
  meal: Meal;
  onDelete: () => void;
  onToggleReminder: () => void;
}) {
  return (
    <motion.div variants={slideUpVariants} whileHover={{ y: -2 }}
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <motion.button onClick={onToggleReminder} whileTap={{ scale: 0.9 }}
          className={`w-10 h-6 rounded-full transition-colors ${meal.reminder ? "bg-[var(--coral)]" : "bg-muted"}`}
          aria-label="Toggle reminder">
          <motion.div layout transition={{ type: "spring", stiffness: 500, damping: 28 }}
...
function Feeding() {
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    addMeal(catId, {
      ...
      food: newFood.trim(), brand: newBrand.trim() || "—",
      amount: newAmount.trim() || "—", reminder: false,
    });
    setNewFood(""); setNewBrand(""); setShowAdd(false);
  }
  ...
  if (!activeCat) return (
    <PhoneShell>
      ...
      <div className="px-6 py-16 flex flex-col items-center text-center">
        ...
        <p className="text-sm text-muted-foreground mb-6 max-w-[240px]">Add a cat from the home page first.</p>
      ...
      <motion.div variants={pageVariants} initial="hidden" animate="visible" exit="exit">
        ...
        <motion.div variants={cardVariants} className="px-6">
          <motion.div whileHover={{ scale: 1.015 }} transition={{ type: "spring", stiffness: 300, damping: 24 }}
            className="rounded-3xl bg-[var(--coral-soft)] p-6 text-center relative overflow-hidden">
            <div className="flex items-center justify-center gap-2 mb-2">
              <motion.div animate={{ rotate: [0, 8, -8, 0] }} transition={{ repeat: Infinity, repeatDelay: 4, duration: 0.5 }}><UtensilsCrossed size={16} /></motion.div>
              <span className="font-serif font-semibold">CatTwin</span>
            </div>
            <p className="font-serif text-2xl font-semibold">
              {meals.length === 0 ? "No meals set" : `${meals.length} meal${meals.length > 1 ? "s" : ""} per day`}
            </p>
            <p className="text-sm text-foreground/70 mt-1">
              {meals.length === 0 ? "Add your first meal below" : `For ${activeCat.name}`}
            </p>
            <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.5, 0.7, 0.5] }}
              transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
              className="absolute -right-6 -bottom-6 w-24 h-24 rounded-full bg-[var(--warm-yellow)]/50 blur-2xl pointer-events-none" />
          </motion.div>
        </motion.div>
          ...
          <AnimatePresence>
            {showAdd ? (
              <motion.form key="form" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                onSubmit={handleAdd} className="mt-4 bg-card rounded-2xl p-4 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)] space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="" className="mt-1 w-full bg-secondary rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--coral)]" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground uppercase tracking-wide">Food <span className="text-[var(--coral)]">*</span></label>
                  <input value={newFood} onChange={(e) => setNewFood(e.target.value)} placeholder="" autoFocus className="mt-1 w-full bg-secondary rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--coral)]" />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-muted-foreground uppercase tracking-wide">Brand</label>
                    <input value={newBrand} onChange={(e) => setNewBrand(e.target.value)} placeholder="" className="mt-1 w-full bg-secondary rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--coral)]" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground uppercase tracking-wide">Amount</label>
                    <input value={newAmount} onChange={(e) => setNewAmount(e.target.value)} placeholder="" className="mt-1 w-full bg-secondary rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--coral)]" />
                  </div>
                </div>
                <div className="flex gap-2 pt-1">
                  <button type="button" onClick={() => setShowAdd(false)} className="flex-1 bg-secondary text-foreground rounded-xl py-2.5 text-sm font-medium">Cancel</button>
                  <button type="submit" disabled={!newFood.trim()} className="flex-1 bg-[var(--nav-dark)] hover:bg-[var(--coral)] text-white rounded-xl py-2.5 text-sm font-medium disabled:opacity-50 transition-colors">Add Meal</button>
                </div>
              </motion.form>
            ) : (
              <motion.button key="btn" whileTap={tapScale} onClick={() => setShowAdd(true)}
