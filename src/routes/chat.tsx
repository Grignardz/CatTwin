...
import { generateReply, SUGGESTED_QUESTIONS } from "@/lib/aiAssistant";
...
function Chat() {
  ...
  function sendMessage(text: string) {
    ...
    setInput("");
    setTyping(true);
    // Simulate a brief "thinking" delay so replies feel considered, then respond.
    setTimeout(() => {
      const reply = generateReply(text, user, activeCat);
      addChatMessage(activeCat.id, { role: "assistant", text: reply.text, actions: reply.actions });
      setTyping(false);
    }, 500 + Math.random() * 400);
  }
      ...
      <motion.div variants={pageVariants} initial="hidden" animate="visible" exit="exit">
        ...
        <motion.div variants={childVariants} className="px-6 pt-12 pb-4 flex items-center gap-3">
          ...
          {activeCat && history.length > 0 && (
            <motion.button whileTap={tapScale} onClick={() => setClearConfirm(true)}
              className="w-9 h-9 rounded-full bg-secondary flex items-center justify-center" aria-label="Clear conversation">
          ...
          {clearConfirm && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              ...
              <div className="bg-red-100 dark:bg-red-900/30 rounded-2xl p-3 flex items-center justify-between gap-3">
                <p className="text-xs text-red-700 dark:text-red-300 flex-1">Clear this conversation?</p>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => setClearConfirm(false)} className="text-xs bg-card px-3 py-1.5 rounded-full font-medium">Cancel</button>
                  <button onClick={handleClearHistory} className="text-xs bg-red-500 text-white px-3 py-1.5 rounded-full font-medium">Clear</button>
                </div>
              </div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {history.length > 4 && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="px-6 mb-2 flex justify-center">
              <motion.button whileTap={tapScale} onClick={() => setCollapsed(!collapsed)}
                className="flex items-center gap-1 text-xs text-muted-foreground bg-secondary rounded-full px-3 py-1">
                <motion.div animate={{ rotate: collapsed ? 0 : 180 }} transition={{ type: "spring", stiffness: 300, damping: 24 }}>
                  <ChevronDown size={12} />
                </motion.div>
              ...
              >
                <motion.div whileHover={{ scale: 1.01 }}
                  className={`max-w-[82%] rounded-2xl p-3 text-sm shadow-sm ${
                    msg.role === "user" ? "rounded-tr-sm bg-[var(--coral-soft)]" : "rounded-tl-sm bg-card"
                  }`}>
                  {msg.text}
                  {msg.actions && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {msg.actions.map((a) => (
                        <motion.div key={a.label} whileTap={tapScale}>
                          <Link to={a.to} className="text-xs bg-[var(--nav-dark)] text-white rounded-full px-3 py-1 font-medium inline-block">
                            {a.label}
                          </Link>
                        </motion.div>
                      ))}
        ...
        <motion.div variants={childVariants} className="px-6 mt-2 flex gap-2 flex-wrap pb-2">
          {SUGGESTED_QUESTIONS.map((p, i) => (
            <motion.button key={p}
              ...
              whileHover={{ scale: 1.03 }} whileTap={tapScale}
        ...
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-[92%] max-w-[380px] z-40">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            ...
            <motion.div whileTap={{ scale: 0.85 }}>
              <Link to="/photo-analysis" className="text-muted-foreground hover:text-foreground transition-colors" aria-label="Photo analysis">
                <Camera size={18} />
              </Link>
            </motion.div>
            <input value={input} onChange={(e) => setInput(e.target.value)}
              ...
              className="flex-1 bg-transparent outline-none text-sm" aria-label="Message input" />
            <motion.button onClick={() => sendMessage(input)} disabled={!input.trim()}
              whileTap={input.trim() ? tapScale : {}}
              animate={input.trim() ? { scale: [1, 1.08, 1] } : { scale: 1 }}
              transition={{ duration: 0.2 }}
              className="w-9 h-9 rounded-full bg-[var(--coral)] flex items-center justify-center disabled:opacity-40" aria-label="Send">
              <Send size={16} />
            </motion.button>
          </motion.div>
        </div>
