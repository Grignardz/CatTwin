...
import {
  ArrowLeft, PawPrint, TrendingUp, TrendingDown, Minus,
  Sparkles, ChevronRight, Syringe, Droplets, Activity as ActivityIcon,
  Moon, UtensilsCrossed, HeartPulse, Weight as WeightIcon, Gauge, Zap,
} from "lucide-react";
...
function ModuleCard({ module }: { module: TwinModule }) {
  ...
  return (
    <motion.div variants={slideUpVariants} className="bg-card rounded-2xl overflow-hidden shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
      <button
        ...
        {open && (
          ...
          >
            <div className="px-4 pb-4">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed mb-2">{module.explanation}</p>
...
function EvolutionTimeline({ points }: { points: { label: string; score: number }[] }) {
  if (points.length === 0) {
    return <div className="h-20 flex items-center justify-center rounded-2xl bg-secondary"><p className="text-sm text-muted-foreground">No history yet</p></div>;
  }
  const max = 100;
  return (
    ...
    return (
      <PhoneShell>
        ...
        <div className="px-6 py-16 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-[240px]">Add a cat to activate their AI Digital Twin.</p>
  ...
  return (
    <PhoneShell>
      <motion.div variants={pageVariants} initial="hidden" animate="visible" exit="exit">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="px-6 space-y-5 pb-8">
          ...
          >
            ...
            {activeCat.photo ? (
              <img src={activeCat.photo} alt={activeCat.name} className="absolute inset-0 w-full h-full object-cover" />
            ) : (
            ...
            <div className="relative p-6">
              <p className="font-serif text-lg font-semibold leading-tight">{activeCat.name}</p>
              ...
              <div className="flex items-center gap-5 mt-4">
                ...
                <div className="flex-1">
                  <p className="text-xs text-white/60 uppercase tracking-wide mb-1">Overall Health Score</p>
                  <p className="text-xs text-white/50 mb-2">AI Confidence: {twin.overallConfidence}%</p>
                  <AnimatedBar pct={twin.overallConfidence} color="rgba(255,255,255,0.5)" />
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            <p className="text-sm text-foreground/80 leading-relaxed">{twin.summary}</p>
          </motion.div>
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            {twin.prediction.predictedWeightKg30d === null ? (
              <p className="text-sm text-muted-foreground text-center py-3">Log at least 2 weight entries to unlock predictions.</p>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-secondary rounded-xl p-3">
                  <p className="text-xs text-muted-foreground mb-1">Predicted Weight</p>
                  <p className="font-serif text-xl font-semibold">{twin.prediction.predictedWeightKg30d}kg</p>
                  <div className="flex items-center gap-1 mt-1">
                    <TrendIcon trend={twin.prediction.obesityRiskDirection} />
                    <span className="text-[11px] text-muted-foreground">
                      {twin.prediction.weightTrendKgPerWeek! >= 0 ? "+" : ""}{twin.prediction.weightTrendKgPerWeek}kg/wk
                    </span>
                  </div>
                </div>
                <div className="bg-secondary rounded-xl p-3">
                  <p className="text-xs text-muted-foreground mb-1">Predicted Score</p>
                  <p className="font-serif text-xl font-semibold">{twin.prediction.predictedHealthScore30d}</p>
                  <p className="text-[11px] text-muted-foreground mt-1">Confidence: {twin.prediction.confidence}%</p>
                </div>
              </div>
            )}
          </motion.div>
