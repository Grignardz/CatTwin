...
import {
  ArrowLeft, Download, PawPrint, TrendingUp, TrendingDown, Minus,
  AlertTriangle, Droplets, Moon, Activity as ActivityIcon, Sparkles, Syringe,
} from "lucide-react";
...
function riskBadge(r: string) {
  ...
  if (r === "medium") return "bg-orange-100 text-orange-600 dark:bg-orange-900/50 dark:text-orange-300";
...
function Analytics() {
  ...
  const allWeightLogs = (user?.weightLogs ?? []).filter((l) => l.catId === catId).sort((a, b) => a.date.localeCompare(b.date));
  const allSleepLogs  = (user?.sleepLogs  ?? []).filter((l) => l.catId === catId).sort((a, b) => a.date.localeCompare(b.date));
  const allHydration  = (user?.hydrationLogs ?? []).filter((h) => h.catId === catId).sort((a, b) => a.date.localeCompare(b.date));
  ...
  const avgSleep = allSleepLogs.length ? allSleepLogs.reduce((s, l) => s + l.hours, 0) / allSleepLogs.length : null;
  ...
  if (latestBcs && latestBcs >= 7) alerts.push({ title: "Obesity Risk", risk: "high", desc: `${activeCat?.name}'s BCS of ${latestBcs}/9 indicates obesity. Consider a vet visit.` });
  if (latestBcs && latestBcs <= 2) alerts.push({ title: "Underweight", risk: "high", desc: `${activeCat?.name}'s BCS of ${latestBcs}/9 suggests they may be underweight.` });
  if (latestActivity === "sedentary") alerts.push({ title: "Low Activity", risk: "medium", desc: `${activeCat?.name}'s activity level is sedentary. Try 2–3 short play sessions daily.` });
  if (avgSleep && avgSleep < 10) alerts.push({ title: "Low Sleep", risk: "low", desc: `Average ${avgSleep.toFixed(1)}h sleep is below the healthy 12–16h range for cats.` });
  ...
  if (!activeCat) return (
    <PhoneShell>
      ...
      <div className="px-6 py-16 flex flex-col items-center text-center">
        ...
        <p className="text-sm text-muted-foreground mb-6 max-w-[240px]">Add a cat and log data to see analytics.</p>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="px-6 space-y-5 pb-6">
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/digital-twin" className="text-xs font-medium text-[var(--coral)] flex items-center gap-1">
                <Sparkles size={11} /> AI Twin
              </Link>
            </div>
            {!hasEnoughData ? (
              <div className="text-center py-4 text-sm text-muted-foreground">Log weight and sleep data to generate a score.</div>
            ) : (
              <div className="flex items-center gap-5">
                <CircularProgress score={twin.overallScore} />
                <div className="flex-1 grid grid-cols-2 gap-2">
                  {[
                    { label: "Weight", val: latestWeight ? `${latestWeight}kg` : "—", ok: bodyMod.hasData ? bodyMod.score >= 70 : null },
                    { label: "Activity", val: latestActivity ?? "—", ok: activityMod.hasData ? activityMod.score >= 60 : null },
                    { label: "Feeding", val: meals.length ? `${meals.length} meals` : "—", ok: meals.length > 0 },
                    { label: "Sleep", val: avgSleep ? `${avgSleep.toFixed(1)}h` : "—", ok: avgSleep ? avgSleep >= 12 && avgSleep <= 16 : null },
                  ].map((m) => (
                    <div key={m.label} className="bg-secondary rounded-xl p-2.5">
                      <div className={`flex items-center gap-1 ${m.ok === true ? "text-emerald-500" : m.ok === false ? "text-red-400" : "text-muted-foreground"}`}>
                        {m.ok === true ? <TrendingUp size={12} /> : m.ok === false ? <TrendingDown size={12} /> : <Minus size={12} />}
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            <div className="flex items-center justify-between mb-2">
              ...
              {weightLogs.length > 0 && <span className="text-xs text-muted-foreground">{weightLogs.length} entries</span>}
            </div>
            <AreaChart data={weightLogs.map((l) => ({ label: l.date.slice(5), value: l.weight }))} color="var(--coral)"
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            <AreaChart data={sleepLogs.map((l) => ({ label: l.date.slice(5), value: l.hours }))} color="#818cf8"
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            <AreaChart
              data={sleepLogs.filter((l) => l.activity).map((l) => ({ label: l.date.slice(5), value: ACTIVITY_LEVEL[l.activity] ?? 0 }))}
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            <AreaChart data={hydration.map((h) => ({ label: h.date.slice(5), value: h.ml }))} color="#38bdf8"
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            {vaccines.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-2">No vaccination records yet.</p>
            ) : (
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-secondary rounded-xl p-3 text-center">
                  <p className="font-serif text-2xl font-semibold text-emerald-500">{vaccComplete}</p>
                ...
                <div className="flex-1 bg-secondary rounded-xl p-3 text-center">
                  <p className="font-serif text-2xl font-semibold text-muted-foreground">{vaccines.length}</p>
          ...
          {/* Risk Alerts */}
          <div>
            <motion.h2 variants={childVariants} className="font-serif text-lg font-semibold mb-3">Risk Alerts</motion.h2>
            <AnimatePresence>
              {alerts.filter((a) => !dismissed.includes(a.title)).map((alert, i) => (
                <motion.div key={alert.title}
                  initial={{ opacity: 0, y: 12, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, x: 60, scale: 0.95 }}
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2"><AlertTriangle size={15} className="text-[var(--warm-yellow)]" /><span className="text-sm font-semibold">{alert.title}</span></div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs rounded-full px-2 py-0.5 font-medium ${riskBadge(alert.risk)}`}>{alert.risk.charAt(0).toUpperCase() + alert.risk.slice(1)}</span>
                      <motion.button whileTap={{ scale: 0.8 }} onClick={() => setDismissed((d) => [...d, alert.title])} className="text-muted-foreground">✕</motion.button>
                    </div>
                  </div>
              ...
              {alerts.length === 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-2xl p-4 text-center text-sm text-muted-foreground">
          ...
          <motion.div variants={cardVariants} className="bg-card rounded-3xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            ...
            <p className="text-xs text-muted-foreground mb-4">vs. average {activeCat.breed || "domestic cat"}</p>
            {!hasEnoughData ? (
              <p className="text-sm text-muted-foreground text-center py-3">Log data to unlock breed comparison.</p>
            ) : (
              <div className="space-y-3">
                {[
                  { label: "BCS (body condition)", val: latestBcs ? `${latestBcs}/9` : "—", pct: latestBcs ? (latestBcs / 9) * 100 : 0, color: "var(--coral)" },
                  { label: "Avg sleep vs healthy", val: avgSleep ? `${avgSleep.toFixed(1)}h` : "—", pct: avgSleep ? Math.min((avgSleep / 14) * 100, 100) : 0, color: "#818cf8" },
                  { label: "Health score", val: hasEnoughData ? `${twin.overallScore}/100` : "—", pct: twin.overallScore, color: "var(--warm-yellow)" },
                ].map((row) => (
                  <div key={row.label}>
          ...
          {/* Export */}
          <motion.button variants={childVariants} whileHover={{ scale: 1.02 }} whileTap={tapScale}
          ...
          {!hasEnoughData && <p className="text-center text-xs text-muted-foreground -mt-3">Available once you have logged data.</p>}
          <Link to="/reports" className="block text-center text-xs font-medium text-[var(--coral)] underline underline-offset-2 -mt-1">
