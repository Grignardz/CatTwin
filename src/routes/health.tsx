...
function MiniLineChart({ data, color }: { data: number[]; color: string }) {
  if (data.length < 2)
    return (
      <div className="h-16 flex items-center justify-center rounded-xl bg-secondary">
        <p className="text-xs text-muted-foreground">Log at least 2 entries to see the trend</p>
      </div>
    );
  ...
  const w = 280,
    h = 80;
  const pts = data.map(
    (v, i) => `${(i / (data.length - 1)) * w},${h - ((v - min) / range) * (h - 10) - 5}`,
  );
  return (
    <svg
      width="100%"
    ...
    >
      ...
      <path
        d={`M ${pts[0]} L ${pts.join(" L ")} L ${w},${h} L 0,${h} Z`}
        ...
        return (
          <div key={d} className="flex-1 flex flex-col items-center gap-1">
            <div
              ...
              style={{ background: `oklch(0.82 0.09 35 / ${opacity})` }}
            />
...
function StatusBadge({ status }: { status: string }) {
  if (status === "complete")
    return (
      <span className="flex items-center gap-1 text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300 rounded-full px-2 py-0.5">
        <CheckCircle2 size={11} /> Done
      </span>
    );
  if (status === "due_soon")
    return (
      <span className="flex items-center gap-1 text-xs bg-orange-100 text-orange-700 dark:bg-orange-900/50 dark:text-orange-300 rounded-full px-2 py-0.5">
        <AlertCircle size={11} /> Due Soon
      </span>
    );
  return (
    <span className="flex items-center gap-1 text-xs bg-secondary text-muted-foreground rounded-full px-2 py-0.5">
      <Circle size={11} /> Not Due
    </span>
  );
}
...
function AddVetForm({
  ...
  const [status, setStatus] = useState<VetRecord["status"]>(isMedical ? "complete" : "not_due");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    addVetRecord(catId, {
      ...
      name: name.trim(),
      date,
      status: isMedical ? "complete" : status,
  ...
  const namePlaceholder =
    type === "vaccine"
      ? "Name (e.g. Rabies) *"
      : type === "deworming"
        ? "Name (e.g. Praziquantel) *"
        : "Name (e.g. Ear infection) *";
  return (
    <form onSubmit={handleSubmit} className="mt-3 bg-secondary rounded-2xl p-3 space-y-2">
      ...
      <div className="flex gap-2 pt-1">
        <button
          type="button"
          onClick={onDone}
          className="flex-1 bg-card border border-border rounded-xl py-2 text-sm font-medium"
        >
          Cancel
        </button>
        <button
          type="submit"
          disabled={!name.trim()}
          className="flex-1 bg-[var(--nav-dark)] text-white rounded-xl py-2 text-sm font-medium disabled:opacity-50"
        >
          Save
        </button>
      </div>
...
function Health() {
  ...
  const sleepLogs = (user?.sleepLogs ?? [])
    .filter((l) => l.catId === catId)
    .sort((a, b) => a.date.localeCompare(b.date));
  const hydrationLogs = (user?.hydrationLogs ?? [])
  ...
  const medicalHistory = vetRecords
  ...
  const sliderColor = () => {
    const p = hoursSlept / 24;
    return p < 0.3 ? "#ef4444" : p < 0.5 ? "#f97316" : "var(--coral)";
  };
      ...
      <PhoneShell>
        ...
        <div className="px-6 py-16 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-[240px]">
            Add a cat from the home page to start tracking health.
          </p>
          <Link
            to="/"
    ...
    <PhoneShell>
      <motion.div variants={pageVariants} initial="hidden" animate="visible" exit="exit">
        <motion.div variants={childVariants} className="px-6 pt-12 pb-4 flex items-center gap-3">
          ...
          <h1 className="font-serif text-xl font-semibold flex-1 text-center pr-9">
            {activeCat.name}'s Health
          </h1>
        </motion.div>
              ...
              >
                ...
                <div className="flex items-center justify-center gap-4 mb-4">
                  <motion.button
                    ...
                    onClick={() =>
                      setWeight((w) => Math.max(0.1, parseFloat((w - 0.1).toFixed(1))))
                    }
                  ...
                  <div className="text-center">
                    <motion.span
                      ...
                      className="font-serif text-5xl font-semibold inline-block"
                    >
                      {weight.toFixed(1)}
                    </motion.span>
                  ...
                  <motion.button
                    ...
                    className="w-12 h-12 rounded-full bg-[var(--coral-soft)] flex items-center justify-center"
                  >
                    <Plus size={20} />
                  </motion.button>
                </div>
                  ...
                  {noteOpen && (
                    ...
                    >
                      <textarea
                        ...
                        placeholder="e.g. Weighed before breakfast..."
                        className="w-full bg-secondary rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--coral)] resize-none mb-3"
                      />
                    </motion.div>
                  )}
                </AnimatePresence>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {(["sedentary", "light", "moderate", "active"] as const).map((lvl) => (
                      <motion.button
                        ...
                        whileTap={tapScale}
                        className={`py-2.5 rounded-xl text-sm font-medium capitalize transition-all ${activity === lvl ? "bg-[var(--coral)] text-white" : "bg-secondary text-foreground"}`}
                      >
                        {lvl}
                      </motion.button>
                    ))}
                  ...
                  <div className="flex items-center gap-1.5 mt-2 mb-1">
                    {Array.from({ length: 9 }, (_, i) => i + 1).map((n) => (
                      <motion.button
                        ...
                        onClick={() => setBcs(n)}
                        whileTap={{ scale: 0.88 }}
                        className={`flex-1 aspect-square rounded-lg text-xs font-medium transition-all ${n === bcs ? "bg-[var(--coral)] text-white" : n < bcs ? "bg-[var(--coral-soft)] text-foreground" : "bg-secondary text-muted-foreground"}`}
                      >
                        {n}
                      </motion.button>
                    ))}
              ...
              >
                ...
                <div className="flex items-center justify-center gap-4 mb-4">
                  <motion.button
                    whileTap={tapScale}
                    whileHover={{ scale: 1.06 }}
                    onClick={() => setWaterMl((w) => Math.max(0, w - 25))}
                    className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center"
                  >
                    <Minus size={20} />
                  </motion.button>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-serif text-lg font-semibold">Vaccination Schedule</h2>
                </div>
                {vaccines.length === 0 ? (
                ...
                {addingVet === "vaccine" ? (
                  <AddVetForm catId={catId} type="vaccine" onDone={() => setAddingVet(null)} />
                ) : (
                  <button
                    onClick={() => setAddingVet("vaccine")}
                    className="mt-4 w-full flex items-center justify-center gap-2 border-2 border-dashed border-border rounded-xl py-2.5 text-sm text-muted-foreground hover:border-[var(--coral)] hover:text-[var(--coral)] transition-colors"
                  >
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-serif text-lg font-semibold">Deworming Schedule</h2>
                </div>
                {dewormings.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-3">
                ...
                {addingVet === "deworming" ? (
                  <AddVetForm catId={catId} type="deworming" onDone={() => setAddingVet(null)} />
                ) : (
                  <button
                    onClick={() => setAddingVet("deworming")}
                    className="mt-4 w-full flex items-center justify-center gap-2 border-2 border-dashed border-border rounded-xl py-2.5 text-sm text-muted-foreground hover:border-[var(--coral)] hover:text-[var(--coral)] transition-colors"
                  >
                    <Plus size={14} /> Add deworming
                  </button>
                )}
              </motion.div>
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-serif text-lg font-semibold">Medical History</h2>
                </div>
                      ...
                      {medicalHistory.map((m, i) => (
                        <motion.div key={m.id} variants={slideUpVariants} className="relative">
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            transition={{ type: "spring", stiffness: 500, delay: 0.08 * i }}
                            className="absolute -left-7 top-1 w-2.5 h-2.5 rounded-full bg-[var(--coral)] border-2 border-background"
                          />
                ...
                {addingVet === "medical" ? (
                  <AddVetForm catId={catId} type="medical" onDone={() => setAddingVet(null)} />
                ) : (
                  <button
                    onClick={() => setAddingVet("medical")}
                    className="mt-4 w-full flex items-center justify-center gap-2 border-2 border-dashed border-border rounded-xl py-2.5 text-sm text-muted-foreground hover:border-[var(--coral)] hover:text-[var(--coral)] transition-colors"
                  >
